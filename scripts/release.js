...
/**
 * Klir Mobile Release & Automated Git Versioning Engine
 *
 ...
 *   2. app.json ("expo.version" and "expo.android.versionCode")
...
function getTotalCommitCount() {
  ...
  return count ? parseInt(count, 10) : 59;
}
...
function main() {
  console.log('════════════════════════════════════════════════════════════════');
  console.log('  KLIR MOBILE • Automated Git Release & Versioning Engine       ');
  console.log('════════════════════════════════════════════════════════════════');
  ...
  const currentVersion = pkg.version || '1.24.4';
  ...
  if (isDryRun) {
    ...
    console.log(`   • app.json                     -> "version": "${nextVersion}", "versionCode": ${nextBuildCode}`);
    console.log(`   • android/app/build.gradle     -> versionName "${nextVersion}", versionCode ${nextBuildCode}`);
  ...
  if (fs.existsSync(BUILD_GRADLE_PATH)) {
    ...
    gradleContent = gradleContent.replace(/versionCode\s+\d+/, `versionCode ${nextBuildCode}`);
  ...
  try {
    const gitAddFiles = ['package.json', 'app.json', 'android/app/build.gradle', 'CHANGELOG.md'];
